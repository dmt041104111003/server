...
var BlockfrostProvider = class {
   ...
   */
  async fetchAddressTxs(address, option = import_common2.DEFAULT_FETCHER_OPTIONS) {
    const txs = [];
    try {
      const fetcherOptions = { ...import_common2.DEFAULT_FETCHER_OPTIONS, ...option };
  ...
  resolveScriptRef = (kScriptRef) => {
    if (kScriptRef) {
      ...
      if (kScriptRef.type.startsWith("plutus")) {
        const normalized = (0, import_core_cst2.normalizePlutusScript)(
          kScriptRef.bytes,
          "DoubleCBOR"
        );
        script = {
...
var U5CProvider = class {
  ...
  onTxConfirmed(txHash, callback, limit = 100) {
    const onConfirmed = async () => {
      const updates = this.submitClient.waitForTx((0, import_common6.hexToBytes)(txHash));
   ...
   */
  async submitTx(tx) {
    const cbor = (0, import_common6.toBytes)(tx);
    const hash = await this.submitClient.submitTx(cbor);
    return (0, import_common6.bytesToHex)(hash);
  }
  /**
   * Obtain information about a specific stake account.
   * @param address - Wallet address to fetch account information
  ...
  awaitTransactionConfirmation(txId, timeout) {
    const onConfirmed = (async () => {
      const updates = this.submitClient.waitForTx((0, import_common6.hexToBytes)(txId));
      for await (const stage of updates) {
        if (stage == import_spec.submit.Stage.CONFIRMED) {
          return true;
        }
...
var YaciProvider = class {
  ...
  async fetchHandleAddress(handle) {
    try {
      const assetName = (0, import_common7.fromUTF8)(handle.replace("$", ""));
      const { data, status } = await this._axiosInstance.get(
        `assets/${import_common7.SUPPORTED_HANDLES[1]}${assetName}/addresses`
      );
...
var OfflineFetcher = class _OfflineFetcher {
  ...
  async fetchHandle(handle) {
    try {
      ...
      const handleAsset = `${import_common8.SUPPORTED_HANDLES[1]}000de140${assetName}`;
  ...
  async fetchHandleAddress(handle) {
    const assetName = (0, import_common8.fromUTF8)(handle.replace("$", ""));
    const policyId = import_common8.SUPPORTED_HANDLES[1];
  ...
  addSerializedTransaction(txHex) {
    const tx = import_core_cst6.Serialization.Transaction.fromCbor(import_core_cst6.Serialization.TxCBOR(txHex));
    const time = Date.now();
    const slot = (0, import_common8.resolveSlotNo)(this.network ?? "mainnet", time);
    const epoch = (0, import_common8.resolveEpochNo)(this.network ?? "mainnet", time);
  ...
  slotToEpochSlot(slot) {
    const slotConfig = import_common9.SLOT_CONFIG_NETWORK[this.network ?? "mainnet"];
