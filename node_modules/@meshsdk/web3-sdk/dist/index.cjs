...
__export(index_exports, {
  ...
  getWalletFromWindow: () => getWalletFromWindow,
  hashData: () => hashData,
...
async function combineShardsBuildWallet(networkId, keyShard1, keyShard2) {
  ...
  const wallet = new import_wallet.MeshWallet({
    networkId,
    key: {
      type: "mnemonic",
      words: key.split(" ")
...
async function clientGenerateWallet(spendingPassword) {
...
var Web3Wallet = class _Web3Wallet extends import_wallet4.MeshWallet {
  projectId;
  appUrl;
  constructor(options) {
    super(options);
...
async function getWalletFromWindow({
  ...
  const payload = { networkId, projectId };
